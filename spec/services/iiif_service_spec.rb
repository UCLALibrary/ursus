  ...
  describe '#src' do
    ...
    context 'by default' do
      it 'links to universal viewer' do
        ...
        expect(service.src(request, solr_document)).to eq 'https://d1eaxpqd9huxux.cloudfront.net/uv.html#?manifest=https%3A%2F%2Fmanifest.store%2Fark%253A%252Fabc%252F123%2Fmanifest'
      end
    end
    ...
    context 'when the link parameter "cv" exists' do
      it 'links to the corresponding Work page in the universal viewer' do
        ...
        expect(service.src(request, solr_document_with_cv)).to eq 'https://d1eaxpqd9huxux.cloudfront.net/uv.html#?cv=7&manifest=https%3A%2F%2Fmanifest.store%2Fark%253A%252Fabc%252F123%2Fmanifest'
      end
    end
  end
end
