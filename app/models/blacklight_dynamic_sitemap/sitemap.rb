    ...
    def max_documents
      ...
      Rails.cache.fetch(key, expires_in: expiration) do
        Blacklight.default_index.connection.select(
          params: { q: '*:*', rows: 0, facet: false, fq: '(((has_model_ssim:Work) OR (has_model_ssim:Collection)) AND !((visibility_ssi:restricted) OR (visibility_ssi:discovery) OR (visibility_ssi:sinai)))'  }
        )['response']['numFound']
      end
    end
