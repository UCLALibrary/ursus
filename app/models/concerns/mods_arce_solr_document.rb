      ...
      xml['mods'].mods('xmlns:mods' => 'http://www.loc.gov/mods/v3', 'version' => '3.8', 'xmlns:xlink' => 'http://www.w3.org/1999/xlink') do
        ...
        if topic_geographic.any? { |topic_geographic_key| self[topic_geographic_key].present? }
          xml['mods'].subject do
            ...
            mods_with_topic = [:subject_cultural_object_tesim, :subject_domain_topic_tesim].map { |name| self[name] || [] }.flatten
            filtered_topic_array = self[:subject_topic_tesim]&.reject { |item| mods_with_topic.include?(item) }
