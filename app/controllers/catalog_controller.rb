...
class CatalogController < ApplicationController
  ...
  configure_blacklight do |config|
    ...
    config.default_solr_params = {
      ...
      fq: '(ark_ssi:* AND ((has_model_ssim:Work) OR (has_model_ssim:Collection)) AND !((visibility_ssi:restricted) OR (visibility_ssi:discovery)))'
      ### we want to only return works where visibility_ssi == open (not restricted)
    }
    config.default_solr_params[:fq] = '((has_model_ssim:Work))'
