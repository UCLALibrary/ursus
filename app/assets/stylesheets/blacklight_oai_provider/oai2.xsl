...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="style"> td.value { vertical-align: top; padding-left: 1em; padding: 3px; }
    td.key { background-color: #e0e0ff; padding: 3px; text-align: right; border: 1px solid #c0c0c0;
    white-space: nowrap; font-weight: bold; vertical-align: top; } .dcdata td.key {
    background-color: #ffffe0; } body { margin: 1em 2em 1em 2em; } h1, h2, h3 { font-family:
    sans-serif; clear: left; } h1 { padding-bottom: 4px; margin-bottom: 0px; } h2 { margin-bottom:
    0.5em; } h3 { margin-bottom: 0.3em; font-size: medium; } .link { border: 1px outset #88f;
    background-color: #c0c0ff; padding: 1px 4px 1px 4px; font-size: 80%; text-decoration: none;
    font-weight: bold; font-family: sans-serif; color: black; } .link:hover { color: red; }
    .link:active { color: red; border: 1px inset #88f; background-color: #a0a0df; } .oaiRecord,
    .oaiRecordTitle { background-color: #f0f0ff; border-style: solid; border-color: #d0d0d0; }
    h2.oaiRecordTitle { background-color: #e0e0ff; font-size: medium; font-weight: bold; padding:
    10px; border-width: 2px 2px 0px 2px; margin: 0px; } .oaiRecord { margin-bottom: 3em;
    border-width: 2px; padding: 10px; } .results { margin-bottom: 1.5em; } ul.quicklinks {
    margin-top: 2px; padding: 4px; text-align: left; border-bottom: 2px solid #ccc; border-top: 2px
    solid #ccc; clear: left; } ul.quicklinks li { font-size: 80%; display: inline; list-stlye: none;
    font-family: sans-serif; } p.intro { font-size: 80%; } <xsl:call-template name='xmlstyle' />
  </xsl:template>
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        ...
        <p class="intro">You are viewing an HTML version of the XML OAI response. To see the
    underlying XML use your web browsers view source option. More information about this XSLT is at
    the <a href="#moreinfo">bottom of the page</a>. </p>
        <xsl:apply-templates select="/oai:OAI-PMH" />
        <xsl:call-template name="quicklinks" />
        <h2>
          <a name="moreinfo">About the XSLT</a>
        </h2>
        <p>An XSLT file has converted the <a href="http://www.openarchives.org">OAI-PMH 2.0</a>
    responses into XHTML which looks nice in a browser which supports XSLT such as Firefox, Chrome,
    Safari, and Internet Explorer. The XSLT file was created by <a
            href="http://www.ecs.soton.ac.uk/people/cjg">Christopher Gutteridge</a> at the
    University of Southampton as part of the <a
            href="http://www.eprints.org/uk/index.php/eprints-software/">GNU EPrints system</a>, and
    is freely redistributable under the <a href="http://www.gnu.org">GPL</a>. </p>
        <p>For more information or to download the XSL file please see the <a
            href="http://wiki.eprints.org/w/EPrints_OAI_Stylesheet">OAI to XHTML XSLT homepage</a>. </p>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template name="quicklinks">
    <ul class="quicklinks">
      ...
      <li>
        <a href="?verb=ListRecords&amp;metadataPrefix=oai_dpla">ListRecords</a> | </li>
      <li>
        <a href="?verb=ListSets">ListSets</a> | </li>
      <li>
        <a href="?verb=ListMetadataFormats">ListMetadataFormats</a> | </li>
      <li>
        <a href="?verb=ListIdentifiers&amp;metadataPrefix=oai_dpla">ListIdentifiers</a>
      </li>
    </ul>
  </xsl:template>
  ...
  <xsl:template match="/oai:OAI-PMH">
    ...
    <xsl:choose>
      <xsl:when test="oai:error">
        <h2>OAI Error(s)</h2>
        <p>The request could not be completed due to the following error or
    errors.</p>
        <div class="results">
  ...
  <xsl:template match="/oai:OAI-PMH/oai:Identify">
    ...
    <xsl:apply-templates
      select="oai:description" />
    <!--no
    warning about unsupported descriptions -->
  </xsl:template>
...
-->
  ...
  <xsl:template match="oai:description/*" priority="-100">
    ...
    <p>The XSL currently does not support this type of
    description.</p>
    <div class="xmlSource">
...
-->
  ...
  <xsl:template match="gw:gatewayAdmin"
    xmlns:gw="http://www.openarchives.org/OAI/2.0/gateway/">
    <tr>
  ...
  <xsl:template match="oai:set">
    ...
    <table class="values">
      ...
      <tr>
        ...
        <td class="value">
          <xsl:choose>
            <xsl:when test="oai:setName = 'Record not found Blacklight::Exceptions::RecordNotFound'">
    ...
    <xsl:choose>
      <xsl:when test="$identifier">
        <p>This is a list of metadata formats available for the record "<xsl:value-of
            select='$identifier' /> ". Use these links to view the metadata: <xsl:apply-templates
  ...
  <table class="values">
      <tr>
        ...
        <td class="value">
          ...
          <xsl:text></xsl:text>
          <a class="link"
            href="?verb=GetRecord&amp;metadataPrefix=oai_dpla&amp;identifier={oai:identifier}">
    oai_dpla</a>
          <xsl:text></xsl:text>
          <a class="link"
            href="?verb=GetRecord&amp;metadataPrefix=oai_dc&amp;identifier={oai:identifier}">oai_dc</a>
          <xsl:text></xsl:text>
          <xsl:if test="oai:setSpec = 'oai_set_ssim:arce'">
            <a class="link"
              href="?verb=GetRecord&amp;metadataPrefix=mods_arce&amp;identifier={oai:identifier}">
    mods_arce</a>
          </xsl:if>
          <xsl:text></xsl:text>
  ...
  <xsl:template match="oai:setSpec">
    <tr>
      ...
      <td class="value">
        <xsl:value-of select="." />
        <xsl:text></xsl:text>
        <a class="link" href="?verb=ListIdentifiers&amp;metadataPrefix=oai_dpla&amp;set={.}">
    Identifiers</a>
        <xsl:text></xsl:text>
        <a class="link" href="?verb=ListRecords&amp;metadataPrefix=oai_dpla&amp;set={.}">Records</a>
      </td>
    </tr>
  </xsl:template>
      ...
      <table class="values">
          <tr>
            ...
            <td class="value">
              <xsl:value-of select="." />
              <xsl:text></xsl:text>
              <a class="link" href="?verb={$verb}&amp;resumptionToken={.}">Resume</a>
            </td>
          </tr>
        </table>
      </xsl:when>
  ...
  <xsl:template match="dcterms:alternative"
    ...
    <tr>
      <td class="key">Alternate Title or SubSeries Title</td>
      <td class="value">
        <xsl:value-of select="." />
      </td>
    </tr>
  ...
  <xsl:template match="edm:object"
    ...
    <tr>
      <td class="key">Representative Image URL</td>
      <td class="value">
        <xsl:value-of select="." />
      </td>
    </tr>
...
<span class="xmlAttrName">
  ...
  <xsl:template name="xmlstyle"> .xmlSource { font-size: 70%; border: solid #c0c0a0 1px;
    ...
    padding-left: 5px; } .xmlAttrValue { color: #0000c0; } </xsl:template>
