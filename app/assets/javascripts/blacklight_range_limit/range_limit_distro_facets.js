...
Blacklight.onLoad(function () {
  ...
  //Comment this line to avoid histogram to be shown
    ...
    function () {
      ...
      $(container).load($(this).attr("href"), function (response, status) {
        if ($(container).hasClass("chart_js") && status == "success") {
          $(".range_limit .profile .distribution.chart_js ul").css(
            "display",
            "none"
          );
          //Comment this line to avoid histogram to be shown
  ...
  $("body").on("show.bs.collapse", function (event) {
    ...
    var container = $(event.target).filter(".facet-field__body").find(".chart_js");
  ...
  function redrawPlot(container) {
    if (container && container.width() > 0) {
      ...
      if (plot) {
        ...
        plot.getPlaceholder;
        // console.log(
        //   "plot placeholder:  name: " + plot.getPlaceholder.prop("nodeName")
        // );
        // console.log(
        //   "plot placeholder: classname : " +
        //     plot.getPlaceholder.prop("className")
        // );
        plot.resize();
        plot.setupGrid();
        plot.draw();
